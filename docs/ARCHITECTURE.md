...
- **Quota classifier and latch** decide whether a Responses request may use fallback and for how long.
...
2. The router buffers a Responses request in memory, capped at 128 MiB, so the same request can be retried once.
3. The official request uses the original ChatGPT headers and target.
4. A non-success response is buffered up to 2 MiB only for quota classification. An ordinary error is returned unchanged.
5. For an SSE success response, the router buffers only the first event or safety boundary. A quota error can trigger fallback only at this boundary.
6. The fallback body validator rejects server-scoped history and missing portable context, preserves all other request fields, keeps or explicitly maps the model, and sets `store: false`.
7. Fallback headers are rebuilt without official Authorization, cookies, browser origins, ChatGPT headers, or OpenAI-scoped headers.
8. A successful fallback response is streamed to Codex. A failure is reduced to a sanitized local error and is never retried automatically.
